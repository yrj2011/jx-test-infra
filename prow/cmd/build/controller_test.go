...
const (
	...
	errorUpdateBuild   = "error-update-build"
...
func (r *fakeReconciler) updateBuild(namespace string, b *buildv1alpha1.Build) (*buildv1alpha1.Build, error) {
	if b == nil {
		return nil, errors.New("nil build")
	}
	if namespace == errorUpdateBuild {
		return nil, errors.New("injected update prowjob error")
	}
	k := key(namespace, b.Name)
	b.Generation++ // For some reason this is always changing.
	if _, present := r.builds[k]; !present {
		return nil, apierrors.NewNotFound(buildv1alpha1.Resource("Build"), b.Name)
	}
	r.builds[k] = *b
	return b, nil
}
...
func TestReconcile(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "update build if spec changes",
			observedJob: &prowjobv1.ProwJob{
				Spec: prowjobv1.ProwJobSpec{
					Agent: prowjobv1.BuildAgent,
					BuildSpec: &buildv1alpha1.BuildSpec{
			...
			observedBuild: func() *buildv1alpha1.Build {
				pj := prowjobv1.ProwJob{}
				pj.Spec.Agent = prowjobv1.BuildAgent
			...
			expectedBuild: func(pj prowjobv1.ProwJob, b buildv1alpha1.Build) buildv1alpha1.Build {
				b.Spec = *pj.Spec.BuildSpec
				b.Generation++
				return b
			},
		},
		...
		{
			name:      "error when we cannot update build",
			namespace: errorUpdateBuild,
			err:       true,
			observedJob: &prowjobv1.ProwJob{
				Spec: prowjobv1.ProwJobSpec{
					Agent: prowjobv1.BuildAgent,
