...
func main() {
	...
	pjif := prowjobinfo.NewSharedInformerFactory(pjc, time.Minute*30)
	bif := buildinfo.NewSharedInformerFactory(bc, time.Minute*30)
...
func newController(kc kubernetes.Interface, pjc prowjobset.Interface, bc buildset.Interface, pji prowjobinfov1.ProwJobInformer, bi buildinfov1alpha1.BuildInformer) *controller {
	...
	// Reconvile whenever a build changes.
...
func (c *controller) runWorker() {
	for {
		...
		if err := reconcile(c, key.(string)); err != nil {
			...
			continue // Do not forget so we retry later.
		}
...
type reconciler interface {
	...
	updateBuild(namespace string, b *buildv1alpha1.Build) (*buildv1alpha1.Build, error)
...
func (c *controller) updateBuild(namespace string, b *buildv1alpha1.Build) (*buildv1alpha1.Build, error) {
	return c.bc.BuildV1alpha1().Builds(namespace).Update(b)
}
...
// reconcile ensures that the prowjob and build
func reconcile(c reconciler, key string) error {
	...
	case pj.Spec.Agent != prowjobv1.BuildAgent:
	...
	pj.Spec.BuildSpec.Generation = b.Spec.Generation
	if !reflect.DeepEqual(b.Spec, *pj.Spec.BuildSpec) {
		logrus.Infof("\n%#v\n!=\n%#v", b.Spec, *pj.Spec.BuildSpec)
		b2 := b.DeepCopy()
		b2.Spec = *pj.Spec.BuildSpec
		logrus.Infof("Update builds/%s", key)
		if b, err = c.updateBuild(namespace, b2); err != nil {
			return fmt.Errorf("update build spec: %v", err)
		}
	}
